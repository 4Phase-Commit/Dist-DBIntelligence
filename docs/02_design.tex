...
request issued and will ignore responses received after it ends. A \texttt{Client} \textit{may not} receive a response for the result of a
...
Upon successfully receiving the \texttt{Synchronization} message, a replica applies all missing updates included in the metadata. Following this, replicas initiate a recovery phase to retrieve any updates that may have been lost due to the coordinator's failure. Specifically, an update might have reached a quorum of replicas without the previous coordinator broadcasting the corresponding \texttt{WriteOK} message before crashing. To resolve such inconsistencies, each replica responds to the synchronization message by sending the new coordinator its list of pending updates, those received from the previous coordinator for which a \texttt{WriteOK} confirmation was missing entirely.
The new coordinator evaluates these lists and broadcasts the set of updates to all operational replicas. Once this state transfer is complete, all updates are applied immediately, and each replica transitions back to \texttt{Normal Mode}, increments the system epoch counter, updates the active coordinator identifier, and clears its local queue of pending updates. This mechanism strictly enforces that no update that is reached from a quorum is lost.
