...
were implemented as queues of \texttt{Cancellable} timeouts. New timeouts with the same purposes are added to a dedicated queue and they fire a timeout message when they
...
expires then the recipient must have crashed, meaning that all other timeouts in the queue will eventually expire too. Previous timeouts are
canceled upon starting a new election.

\subsubsection{Update protocol}
Write requests received by clients are assigned an ID, attached the sender's \texttt{ActorRef} and added to a queue, where they are sent one
at a time to the coordinator (or accumulated if an election is in progress). Requests sent are moved to a second queue to keep track of
messages that might need to be re-sent. After an election the requests in the second queue are added in front of the original queue to re-attempt sending them to the coordinator.

All replicas, when receiving an update message, add the update to a "pending" queue and reply with an \texttt{UPDATEACK} that contains the
update's ID. They also check if that update is the first request in their second queue and if so, they remove it.
The coordinator also performs these operations like any other replica.
...
The coordinator, upon receiving enough ACKs to reach the quorum, broadcasts an empty \texttt{WRITEOK} message. When a replica receives a 
\texttt{WRITEOK}, it applies the first update in the "pending" queue and adds it to its history (attaching the epoch number and sequence
number for potential future recoveries). The replica that first sent the request Will also send a \texttt{WriteResult} to the client.
Timeouts are in place to detect a potential coordinator crash during these operations.

\subsubsection{Election protocol}
Upon receiving an \texttt{ElectionStarted} message, a replica removes the current coordinator from its active list. To prevent redundant or overlapping election cycles, any replica that receives this message sets an internal flag to true, thereby ignoring subsequent \texttt{ElectionStarted} signals.
As outlined in the system design, only the replica with the smallest index is authorized to immediately initiate the actual election process. The remaining replicas defer their action by a timeout calculated dynamically as:
\begin{equation}
\text{Timeout} = 400 \times \text{replica\_id} \text{ ms}
\end{equation}
...
\begin{itemize}
    ...
    \item If the failure occurs in the second phase, it means the crashed replica had already registered its values. To prevent an entire restart of the election cycle, the replica removes the entries associated with the crashed node from the metadata and forwards the modified message to the subsequent node.
\end{itemize}
...
If the calculated coordinator ID matches the replica's own ID, the replica promotes itself to the coordinator role by setting its internal coordinator flag to \texttt{true} and incrementing the system epoch. To optimize network bandwidth, we implemented a personalized update mechanism for the synchronization phase: rather than broadcasting the entire state history, the new coordinator constructs tailored synchronization messages containing only the specific missing entries required by each active replica. For instance, if a replica missed the last two updates, its synchronization message will contain strictly those two entries, whereas a replica missing only a single update will receive solely that specific payload.

If the calculated ID belongs to another node, the replica forwards the unmodified \texttt{Election} message to the next hop in the ring. It then starts an system size based timeout to await the incoming \texttt{Synchronization} message. If this timer expires without receiving the message, the replica assumes the newly elected coordinator crashed immediately after election, thereby triggering a new election cycle.

Upon receiving the final \texttt{Synchronization} message, the replicas apply the personalized updates received.
To avoid a situation where an update is acknowledged by a quorum but is then lost before being confirmed by any replica, the replicas send a copy of their pending updates queue to the coordinator. The coordinator then creates a union of all the queues received and removes any update that has already been applied during the Synchronization. This final queue is sent to all replicas and the updates in it will be applied in the new epoch.
Only then the replicas will revert back to \texttt{Normal Mode}, empty their pending queues and resume their operations like normal. This is safe because if an update was acknowledge by a quorum then at least one correct replica has it, either in its history or in its pending queue. 
...
\end{enumerate}
Once the incoming message count matches the crash trigger threshold, the replica executes an immediate crash. Note that this behavior also applies when broadcasting messages, thereby enabling the testing of more complex edge cases and specific failure scenarios.
